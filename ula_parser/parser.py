...
from syntax_tree.ast import *
from lexer.lexer import keywords, chars


class Parser():
    def __init__(self,module, builder, print_func):
        
        token_list = [x[0] for x in keywords]
        token_list.extend([x[0] for x in chars])
    ...
    def parse(self):
        ...
        def rel(p):
            ...
            if p[1].gettokentype() == 'BIGGER':
                ...
                raise AssertionError('¡Error! El operador que estás utilizando no existe', p[1])
        ...
        @self.pg.production('factor : DIGIT')
        def factor_number(p):
